...
public class TcpClient {
  ...
  public TcpClient(String fileName, String endIpServer, int udpPortServer, int windowSize, int timeout, int tcpMss,
      int dupack, float lp) {
  ...
  public void initClient() {
    try {
      ...
      DatagramSocket socket = new DatagramSocket(); //open datagram socket
      InetAddress address = InetAddress.getByName(endIpServer); //get server address
      ...
      startTransference(sendData, socket, address);
      showStatistics();
  ...
  public void startTransference(byte[] sendData, DatagramSocket socket, InetAddress address) throws Exception {
    // For as each message we will create
    for (int i = 0; i < sendData.length; i = i + (tcpMss - 3)) {

      DatagramPacket packageToSend = mountMessage(i, sendData, address);
          ...
          while (!ackRecievedCorrect) {
            ...
            if (ackPacketReceived) {
              ...
              for (int y = 0; y != (sequenceNumber - lastAckedSequenceNumber); y++) {
                byte[] resendMessage = new byte[tcpMss];
    ...
    checkForAnACK(socket, address);

  }



  public void checkForAnACK(DatagramSocket socket, InetAddress address) throws Exception {
  ...
  public void showStatistics() {
    ...
    // System.out.println("File size: " + fileSizeKB + "KB, Transfer time: " + 0 + "
    // seconds. Throughput: "
    // + throughput + "KBps");
    System.out.println("Number of retransmissions: " + retransmissionCounter);
  }

  public DatagramPacket mountMessage(int i, byte[] sendData, InetAddress address) {
    sequenceNumber += 1; // Increment sequence number
    byte[] message = new byte[tcpMss]; // new byte array for message
    ...
    sentMessageList.add(message); // Add the message to the sent message list
